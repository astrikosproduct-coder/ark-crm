"""
Deals gain PO Received Date — the day a deal is won.

    python po_received_date_metadata.py            # dry run: validates, writes nothing
    python po_received_date_metadata.py --apply    # place the field and publish a version
    python po_received_date_metadata.py --revert   # remove the placement and publish

Run AFTER `alembic upgrade head` — 0034 adds deals.po_received_date.

Decided 21 Sep 2026: Won is the day the PO arrives, and the Dashboard counts
Won by this field alone. It sits directly after PO / LOI Reference and takes
that field's rules exactly — captured at Stage 7, Mandatory on the same terms —
so the reference and its date are asked for together and enforced together.

Booking Date's description said "no field records that yet"; it now names this
one. NO DDL.
"""
...
MODULE = "deals"
API_NAME = "po_received_date"
ANCHOR = "po_loi_reference"

LABEL = "PO Received Date"
DESCRIPTION = (
    "The day the client's purchase order or award letter reached us. This is the day "
    "the deal was won: the Dashboard counts Won by this date."
)
USE_CASE = "Dashboard Won — decided 21 Sep 2026. The date beside PO / LOI Reference."

BOOKING_DATE_DESCRIPTION = (
    "The day the order was booked internally. It is NOT the day the deal was won — "
    "that is PO Received Date, which usually comes first."
)
...
NOTES = {
    "apply": "Deals: PO Received Date added after PO / LOI Reference — the Won date (migration 0034)",
    "revert": "Deals: PO Received Date removed",
}
...
def apply(db) -> list[str]:
    ...
    booking = placement(db, "booking_date")
    if booking is not None:
        booking_def = definition_for(db, booking)
        if booking_def.description != BOOKING_DATE_DESCRIPTION:
            booking_def.description = BOOKING_DATE_DESCRIPTION
            log.append(f"{MODULE}.booking_date: description now names PO Received Date as the Won date")
    return log
